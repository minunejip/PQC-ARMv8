...
// 간단한 AES-128 키 스케줄 구현
// key: 16바이트 AES 키
// round_keys: [0..10] 총 11개의 라운드 키(각각 16바이트)
...
//     // rank별 bytes_prng_public 계산 후
//     uint64_t bytes_needed = ((bytes_prng_public + 15)/16)*16;
//     assert(bytes_needed >= bytes_prng_public);

//     uint8_t temp_buf[bytes_needed];

//     // aes128_ctr_multi_asm 호출
...
// rank 4 정상 동작 코드
// void asm_hash_aes128(uint8_t* pt_seed_array, uint8_t* pt_output_array) {
//     uint8_t iv[16] = {0};   // const 제거 완료
//     uint8_t round_keys[11][16];
    
//     // 표준 AES 키 스케줄 수행
//     aes128_key_schedule(pt_seed_array, round_keys);
    
//     // aes128_ctr_4x_asm 함수 호출로 변경
//     aes128_ctr_4x_asm(pt_output_array, bytes_prng_public, iv, (const uint8_t*)round_keys);
